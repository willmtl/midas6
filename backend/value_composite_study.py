#!/usr/bin/env python3
"""VALUE-METRIC COMBO — the old bakeoff (top10_div2x engine) had P/B beat EV/EBIT > earnings-yield > FCF-yield.
Re-test on the NEW flagship (usca_small + skip_wide) AND test a COMPOSITE (within-sector percentile-rank average
of P/B + earnings-yield + FCF-yield + EV/EBIT + P/S) to see if blending independent-error metrics beats P/B alone.
All components are PIT (FinancialReport: revenue, net_income, free_cash_flow, operating_income, cash_and_equivalents).
-> BacktestResult[value_composite] + JSON.
Run: MSYS_NO_PATHCONV=1 docker exec rotation-celery-worker-1 python -u /app/value_composite_study.py
"""
...
from speculative_sector_study import WIDE   # skip_wide sector set (the new flagship base)

OUT = Path(__file__).resolve().parent / ".data" / "studies" / "value_composite.json"
METRICS = ["pb", "earnings_yield", "fcf_yield", "ev_ebit", "ps", "composite"]
...
def main():
    ...
    sh, eq, ni, dtb, rev, fcf, opinc, cash = (_pit_monthly_panel(reps, f, midx) for f in
        ("shares_outstanding", "total_equity", "net_income", "total_debt",
         "revenue", "free_cash_flow", "operating_income", "cash_and_equivalents"))
    common = stock_m.columns.intersection(sh.columns).intersection(eq.columns)
    R = lambda p: p.reindex(index=midx, columns=common)
    px = stock_m[common]
    sh, eq, ni, dtb, rev, fcf, opinc, cash = (R(x) for x in (sh, eq, ni, dtb, rev, fcf, opinc, cash))
    as_traded = price_basis.as_traded_close(px)
    mktcap = as_traded * sh                       # LOCAL-currency mktcap -> all value ratios are currency-neutral
    pb = mktcap / eq.where(eq != 0)
    ev = mktcap + dtb - cash
    ev_ebit = ev / opinc.where(opinc > 0)         # only positive EBIT
    ey = ni / mktcap.where(mktcap > 0)            # earnings yield (higher better)
    fcfy = fcf / mktcap.where(mktcap > 0)         # fcf yield (higher better)
    ps = mktcap / rev.where(rev > 0)              # price/sales (lower better)
    ...
    def rank_key(cands, date, metric):
        """Return {ticker: sort_key} where LOWER = cheaper/better (for min())."""
        if metric == "pb":
            return {h: pb.loc[date, h] for h in cands}
        if metric == "earnings_yield":
            return {h: -(ey.loc[date, h] if pd.notna(ey.loc[date, h]) else -9e9) for h in cands}
        if metric == "fcf_yield":
            return {h: -(fcfy.loc[date, h] if pd.notna(fcfy.loc[date, h]) else -9e9) for h in cands}
        if metric == "ev_ebit":
            return {h: (ev_ebit.loc[date, h] if pd.notna(ev_ebit.loc[date, h]) else 9e9) for h in cands}
        if metric == "ps":
            return {h: (ps.loc[date, h] if pd.notna(ps.loc[date, h]) else 9e9) for h in cands}
        # composite: average within-pool percentile rank across the 5 metrics (missing -> worst rank 1.0)
        comps = {"pb": pb, "ev_ebit": ev_ebit, "ps": ps}      # lower better
        highs = {"ey": ey, "fcfy": fcfy}                       # higher better
        score = {h: [] for h in cands}
        for _, panel in comps.items():
            vals = {h: panel.loc[date, h] for h in cands}
            order = sorted([h for h in cands if pd.notna(vals[h])], key=lambda h: vals[h])
            n = len(order)
            for i, h in enumerate(order):
                score[h].append(i / max(n - 1, 1))
            for h in cands:
                if pd.isna(vals[h]):
                    score[h].append(1.0)
        for _, panel in highs.items():
            vals = {h: panel.loc[date, h] for h in cands}
            order = sorted([h for h in cands if pd.notna(vals[h])], key=lambda h: -vals[h])
            n = len(order)
            for i, h in enumerate(order):
            ...
            for h in cands:
                if pd.isna(vals[h]):
                    score[h].append(1.0)
        return {h: float(np.mean(score[h])) for h in cands}
    ...
    def run(metric):
        ...
        for i in range(9, len(midx) - 1):
            ...
            for etf in top:
                if str(etf) in WIDE:                          # skip_wide base (the new flagship)
                    continue
                ...
                if not g:
                    continue
                keys = rank_key(g, date, metric)
                p = min(g, key=lambda h: keys[h]); held.add(p)
        ...
        return dict(total=round(tot, 1), annual=round(ann, 1), sharpe=round(shp, 2), dd=round(dd, 1),
                    t=round(_tstat_from_returns(list(r)) or 0, 2), months=n)

    results = {m: run(m) for m in METRICS}
    print("=== VALUE-METRIC COMBO (usca_small + skip_wide base; within-sector selector) ===", flush=True)
    print(f"  {'metric':<16}{'total':>9}{'annual':>8}{'Sharpe':>8}{'DD':>8}{'t':>7}", flush=True)
    for m in METRICS:
        r = results[m]
        print(f"  {m:<16}{r['total']:>8}%{r['annual']:>7}%{r['sharpe']:>8}{r['dd']:>7}%{r['t']:>7}", flush=True)
    best = max(results, key=lambda m: results[m]["total"])
    print(f"\n  best metric by return: {best} {results[best]['total']}%  (P/B alone {results['pb']['total']}%)", flush=True)

    payload = {"results": results, "base": "usca_small + skip_wide", "metrics": METRICS}
    OUT.parent.mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps(payload, indent=2, default=str))
    try:
        from core.models import BacktestResult
        from django.utils import timezone
        BacktestResult.objects.update_or_create(kind="value_composite",
                                                 defaults={"payload": json.loads(json.dumps(payload, default=str)), "computed_at": timezone.now()})
        print("Saved BacktestResult[value_composite]", flush=True)
    except Exception as e:
        print("DB save failed:", e, flush=True)
    print("DONE_VALCOMP", flush=True)
