...
def get_exps(sym):
    ...
    try:
        ...
        expcache[sym]=sorted({x for x in ed[col].tolist()})
    except Exception:
        expcache[sym]=[]
    return expcache[sym]
def pc_oi(sym, d):
...
for mode,fwd,tk,dd,pcv in events:
    po=pc_oi(tk,dd)
    if po is not None:
        OptionSnapshot.objects.update_or_create(ticker=tk,date=dd,source='thetadata_oi',defaults=dict(pc_oi=round(po,3)))
